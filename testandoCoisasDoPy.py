...
def bt2_click():
    janela.filename =  filedialog.askopenfilename(initialdir = "/",title = "Select file",filetypes = 
    ...
    if (True):
        ...
        janela.filename = str(janela.filename)
        im = cv2.imread(janela.filename, cv2.IMREAD_COLOR)
        ...
        template = getAnswer(contours, imReg)
        ...
        for f in files:
            if(True):
                ...
                prova = getAnswer(contours, imReg)
...
#Definindo os botões
bt1=Button(janela, width=20, height=6, text="Selecione o gabarito")
bt1.place(x=200, y=130)

bt2=Button(janela, width=10, height=3, text="Iniciar", command=bt2_click)
bt2.place(x=500, y=100)

bt3=Button(janela, width=10, height=3, text="Parar")
