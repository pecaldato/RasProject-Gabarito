class Respostas:
    ...
    def get_answers(self, contours):
        ...
        for c in self.contours:
            ...
            if 100<cv2.contourArea(c)<300:
                ...
                if(self.cX<=760 or self.cY>=625):
                    for x in range(0, 20):
                        if (cX >= self.ListX[x][0] and cX <= self.ListX[x][1]):
                            for j in range(0, 26):
                                if (cY >= LicontourstY[j][0] and cY <= ListY[j][1]):
                                    ...
                                    self.listRet.append(letra)
                                    self.listNumbers.append(numero)
                                    cv2.putText(imReg, numero+' '+letra, (cX - 25, cY),cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)
                
                else:
                    for x in range(0,10):
                        if(self.cX >= self.RAx[x][0] and self.cX <= self.RAx[x][1]):
                            for j in range(0,4):
                                if(self.cY >= self.RAy[j][0] and self.cY <= self.RAy[j][1]):
                                    self.listRA.append(str(x))
        ...
        if (ctd == len(listNumbers)):
            break
        x = ctd
        self.listOrder.append(aux)
        self.listRA = self.listRA.reverse()       
        return self.listRA, self.listOrder

    #método da construtora
    def __init__(self, contours):

        #pega o vetor de respostas do gabarito
        _,self.gabarito = get_listas(contours)

        #define os ranges que serão utilizados para localizar as questões/respostas
        self.ListX = [[166,203],[204,239],[240,276],[277,312],[313,349],[426,463],[464,499],[500,536],[537,572],[573,609],[686,723],
  [724,759],[760,796],[797,832],[833,869],[946,983],[984,1019],[1020,1056],[1057,1092],[1093,1129]]
        self.ListY = ListY = [[668,705],[706,741],[742,778],[779,814],[815,851],[852,888],[889,924],[925,961],[962,997],[998,1034],[1035,1071],
  ...
  [1548,1583],[1584,1620],[1621,1656]]
        ...
        self.RAx = [[798,835],[836,871],[872,908], [909,944], [945,981], [982,1018],[1019,1054],[1055,1091],[1092,1127],[1128,1164]]
        self.RAy = [[448,484],[485,521],[522,557],[558,594]]
    ...
    def compare_answers(self, respostas):
        ...
        for x in range(len(respostas)):
            if (len(respostas[x]) > 1) or respostas[x] != template[x]:
                wrongAnswer.append([str(x+1), respostas[x]])
            else:
                correctAnswers.append([str(x+1), Respostas[x]])
        Answers = {'correctAnswers' = self.correctAnswers, 'wrongAnswers:' = self.wrongAnswers}
        return Answers
